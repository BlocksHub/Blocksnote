...
test.each([
  ...
  expect(Instance.cleanUrl(input)).toBeOneOf(["https://demo.index-education.net/pronote/", "http://demo.index-education.net/pronote/", "https://demo.index-education.net/"])
})

const instance = await Instance.createFromURL("demo.index-education.net/pronote")

test("should initialize the auth flow", async () => {
  expect(
    instance.source === "https://demo.index-education.net/pronote/"
    && instance.workspaces.every(item => item.delegated === false)
