...
export async function searchSchoolsWithGeo(lat: number, lon: number, limit = 20): Promise<School[]> {
  ...
  return response.data.slice(0, limit).map((item) => new School(item.url, item.nomEtab, Number(item.lat), Number(item.long), item.cp))
}
